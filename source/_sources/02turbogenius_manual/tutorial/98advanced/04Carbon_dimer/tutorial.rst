...
.. _turbogeniustutorial_9804_00:

00 Introduction
----------------------------------------------------------------

In this tutorial, you will compute all-electron VMC and LRDMC energies for the C\ :sub:`2` dimer using several wave-function ansätze:
...
.. warning::
    ...
    In this tutorial, d\ :sub:`C-C` = 2.300 bohr is used just for simplicity. So, please change it if you need a more accurate result.
...
.. _turbogeniustutorial_9804_01:

01 C\ :sub:`2` dimer and C atom - JDFT ansatz
----------------------------------------------------------------

.. _turbogeniustutorial_9804_01_01:
...
C\ :sub:`2` dimer
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Download C\ :sub:`2` dimer structure :download:`C2.xyz <data/C2.xyz>`.

.. literalinclude:: data/C2.xyz
   :language: text
...
C atom
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Download C atom structure :download:`C.xyz <data/C.xyz>`.

.. literalinclude:: data/C.xyz
   :language: text
...
.. _turbogeniustutorial_9804_01_02:
...
.. _turbogeniustutorial_9804_01_03:
...
.. literalinclude:: data/pseudo.dat
   :language: text

.. _turbogeniustutorial_9804_01_04:
...
.. _turbogeniustutorial_9804_01_05:
...
C atom
 .. code-block:: console
    ...
    % turbogenius prep -g -grid 0.10 0.10 0.10 -lbox 12.0 12.0 12.0 -smear 0.01
...
.. code-block:: console
    ...
    % export TURBOPREP_RUN_COMMAND="mpirun -np 16 prep-mpi.x"
    % turbogenius prep -r

See Note in the :ref:`optimization step in 01Hydrogen_dimer <turbogeniustutorial_0101_02>` for the ways to run the calculations.
...
.. _turbogeniustutorial_9804_01_06:
...
.. _turbogeniustutorial_9804_01_07:
...
.. _turbogeniustutorial_9804_01_08:
...
.. code-block:: console
   ...
   % cd .. # if you are in '.../04lrdmc/alat_0.10'
...
.. _turbogeniustutorial_9804_02:

02 C\ :sub:`2` dimer and C atom - JsAGPs ansatz
----------------------------------------------------------------

.. _turbogeniustutorial_9804_02_01:
...
Please refer to the :ref:`Hydrogen tutorial <turbogeniustutorial_9801_06>` for the details.
...
.. code-block:: console
    ...
    % turbogenius vmcopt -g -opt_onebody -opt_twobody -opt_jas_mat -opt_det_mat -optimizer lr -vmcoptsteps 600 -steps 400 -nw 128
...
.. code-block:: console
    ...
    % cd ../01_03convert_WF_JSD_to_JAGP/
...
.. _turbogeniustutorial_9804_02_02:

02-02 VMC-optimization (WF=JsAGPs)
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Please refer to the :ref:`Hydrogen tutorial <turbogeniustutorial_9801_08>` for the details.
Here, only the commands are shown.
...
C\ :sub:`2` dimer
 .. code-block:: console
    ...
    % cp ../01convert_WF_JSD_to_JAGP/pseudo.dat .
...
C atom
 .. code-block:: console
    ...
    % cd ../02optimization/
    % cp ../01_03convert_WF_JSD_to_JAGP/fort.10 .
    % cp ../01_03convert_WF_JSD_to_JAGP/pseudo.dat .
    % cp fort.10 fort.10_jdft
...
.. _turbogeniustutorial_9804_02_03:

02-03 VMC (WF=JsAGPs)
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Please refer to the :ref:`Hydrogen tutorial <turbogeniustutorial_9801_09>` for the details.
Here, only needed commands are shown.
...
.. _turbogeniustutorial_9804_02_04:

02-04 LRDMC (WF=JsAGPs)
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Please refer to the :ref:`Hydrogen tutorial <turbogeniustutorial_9801_10>` for the details.
Here, only the commands are shown.

We will perform LRDMC calculations at several lattice spacings `alat`, and estimate an unbiased energy at `alat`:math:`\to 0`.
For an example, we choose alat=0.20 in the following.
First, copy the wavefunction and pseudopotential files to a work directory:
...
These steps are repeated for several values of alat.
A shell script to do the calculation and extrapolation will be given as follows:

.. code-block:: console

   % cd .. # if you are in '.../04lrdmc/alat_0.20'
...
.. _turbogeniustutorial_9804_03:


03 C\ :sub:`2` dimer and C atom - JAGPu ansatz
----------------------------------------------------------------

.. _turbogeniustutorial_9804_03_01:
...
.. _turbogeniustutorial_9804_03_02:
...
.. _turbogeniustutorial_9804_03_03:
...
.. image:: image/C2_dft_mag.png
   :scale: 100%
   :align: center

Thus, one can obtain an AFM trial wavefunction.

.. _turbogeniustutorial_9804_03_04:
...
.. _turbogeniustutorial_9804_03_05:
...
.. _turbogeniustutorial_9804_03_06:
...
1. VMC
  ...
  The estimated energy values are found in `pip0.d`. They look like as follows:
...
2. LRDMC for a single value of `alat`

  .. code-block:: console

      % cd ../05lrdmc/alat_0.20/
      % cp ../../04vmc/fort.10 .
      % cp ../../04vmc/pseudo.dat .
...
3. LRDMC extrapolation
   ...
   .. code-block:: console
      ...
      % cd .. # if you are in '.../05lrdmc/alat_0.20'
...
.. _turbogeniustutorial_9804_04:

04 C\ :sub:`2` dimer and C atom - JAGP (JPf) ansatz
----------------------------------------------------------------

.. _turbogeniustutorial_9804_04_01:

The most important procedure in a Pfaffian calculation is to convert a JDFT or JAGPu ansatz to JAGP(JPf) ansatz.
Since the JAGP ansatz is a special case of the JPf one, where only :math:`G_{ud}` and :math:`G_{du}` terms are defined as described in the section review_ paper, the conversion can be realized just by direct substitution.
Therefore, the main challenge is to find a reasonable initialization for the two spin-triplet sectors :math:`G_{uu}` and :math:`G_{dd}` that are not described in the JAGP and that otherwise have to be set to 0.
...
(ii) The second approach that also works in a spin-unpolarized case is to determine a standard broken symmetry single determinant ansatz ({\it e.g.}, by TurboRVB built-in DFT within the LSDA) and modify it with a global  spin rotation.
Indeed, in the presence of finite local magnetic moments, it is often convenient to rotate the spin moments of the WF in a direction perpendicular to the spin quantization axis chosen for our spin-dependent Jastrow factor, {\it i.e.}, the :math:`z` quantization axis.
In this way one can obtain reasonable initializations for  :math:`G_{uu}` and :math:`G_{dd}`.
TurboRVB allows every possible rotation, including an arbitrary small one close to the identity.
A particularly important case is when a rotation of :math:`\pi/2` is applied around the :math:`y` direction.
This operation maps

.. math::

   \ket{\uparrow} \rightarrow \frac{1} {\sqrt{2}} \left( \ket{\uparrow} + \ket{\downarrow} \right)  \quad\text{and}\quad  \ket{\downarrow} \rightarrow  \frac 1 {\sqrt{2}} \left( \ket{\uparrow} - \ket{\downarrow} \right).

One can convert from a AGP the pairing function that is obtained from a VMC optimization

.. math::

   {g_{ud}}(\mathbf{i},\mathbf{j}) = {f_S}({{\mathbf{r}}_i},{{\mathbf{r}}_j})\frac{\ket{\uparrow\downarrow}  - \ket{\downarrow\uparrow}}{{\sqrt 2 }} + {f_T}({{\mathbf{r}}_i},{{\mathbf{r}}_j})\frac{\ket{\uparrow\downarrow} + \ket{\downarrow\uparrow}}{{\sqrt 2 }}

to a Pf one

.. math::

   {g_{ud}}(\mathbf{i},\mathbf{j}) \to g\left( {\mathbf{i},\mathbf{j}} \right){\text{ }} = {f_S}({{\mathbf{r}}_i},{{\mathbf{r}}_j})\frac{\ket{\uparrow\downarrow} - \ket{\downarrow\uparrow}}{{\sqrt 2 }} + {f_T}({{\mathbf{r}}_i},{{\mathbf{r}}_j})\left( \ket{\uparrow\uparrow} - \ket{\downarrow\downarrow} \right).
...
.. _turbogeniustutorial_9804_04_03:
...
1. Optimization
  ...
  C\ :sub:`2` dimer
   .. code-block:: console
      ...
      % cd ../02optimization/
      % cp ../01convert_WF_JAGPu_to_JAGP/fort.10 .
      % cp ../01convert_WF_JAGPu_to_JAGP/pseudo.dat .
      % cp fort.10 fort.10_jdft
  ...
  C atom
   .. code-block:: console
      ...
      % cd ../02optimization/
      % cp ../01convert_WF_JAGPu_to_JAGP/fort.10 fort.10
      % cp ../01convert_WF_JAGPu_to_JAGP/pseudo.dat ./
      % cp fort.10 fort.10_jdft

  .. code-block:: console

      % turbogenius vmcopt -g -opt_onebody -opt_twobody -opt_jas_mat -opt_det_mat -optimizer lr -vmcoptsteps 100 -steps 400 -nw 128 -learn 0.30 -num_opt_param 10 -maxtime 86000
