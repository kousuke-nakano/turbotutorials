...
as a consequence of the statistics of fermionic particles, thus :math:`G` is skew-symmetric (i.e., :math:`G^T = -G`, being :math:`^T` the transpose operator), so the diagonal is zero and the number of independent entries is :math:`\sum_{n=1}^{N-1} n = N(N-1)/2`.
